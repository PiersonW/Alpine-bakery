...
// multi-photo support existed. Also parses the stored options JSON (size/
// flavor choices) into a usable object, if the product has any.
function parseImages(row) {
  ...
  try {
    optionGroup = row.options ? JSON.parse(row.options) : null;
  } catch (e) {
    optionGroup = null;
  }

  return { ...row, images, optionGroup };
}
